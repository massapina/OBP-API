...
class MappedEntitlementTest extends ServerSetup {
  ...
  feature("Getting Entitlement data") {
    scenario("We try to get Entitlement") {
      ...
      Entitlement.entitlement.vend.getEntitlements().openOr(List()).size must equal(0)
  ...
  scenario("A Entitlement exists for user and we try to get it") {
    ...
    Entitlement.entitlement.vend.getEntitlement(bankId1, userId1, role1.toString).isDefined must equal(true)
    ...
    foundThing must equal(entitlement1)

    And("Primary id must be UUID")
    foundThing.map(_.entitlementId).mkString.replace("-", "").size must equal(32)
  }
  ...
  scenario("We try to get all Entitlement rows and then delete they"){
    ...
    foundThing1 must equal(entitlement1.toList)
    ...
    foundThing2 must equal(entitlement2.toList)

    And("We try to delete all rows")
    found.foreach {
      d => {
        Entitlement.entitlement.vend.deleteEntitlement(Full(d)) must equal(Full(true))
      }
    }
  }
